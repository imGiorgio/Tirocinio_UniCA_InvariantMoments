...
path = uigetdir;
pattern = fullfile(path, word); % -------- dovrebbe essere automatizzato con l'utilizzo di word --------
immagini = dir(pattern);      

if(ischar(path))
    a = dir(path);                                      % recupero il contenuto della cartella attraverso il path e la funzione 'dir'
    imageVector = {a(:).name}';                         % recupero tutti i nomi dei vari file presenti nella cartella e li salvo in un array
    imageVector(ismember(imageVector,{'.','..'})) = []; % rimuovo eventuali 'punti' o 'doppi punti' che non sono necessari
end
...
for k = 1:length(immagini)
    ...
    nomeRiga(k, :) = {baseFileName}; %salvo dentro un vettore tutti i nomi delle immagini               
    switch moment
        case 'HU'                  
...
for k = 1:length(immagini)
    classeCartella = extractBetween(imageVector{k}, "_", ".");
    classeCartella = str2num(classeCartella{1:1});

    switch classeCartella
        case 1
            count1 = count1 + 1;
        case 2
            count2 = count2 + 1;
        case 3
            count3 = count3 + 1;
        %case 2
            %count4 = count4 + 1; %counter per OASIS
    end             
end


% ------------- Formula per il calcolo di ARP -------------        
for i = 1:length(immagini)
    rDB = 0;

    %recupero la classe dell'i-esima immagine del dataset
    classeImmagine = extractBetween(imageVector{i}, "_", "."); 
    classeImmagine = str2num(classeImmagine{1:1});

    imageFeatures = folderFeatures(i,:);

    for j = 1:length(immagini)
        baseFileName = immagini(j).name;
        nomeRiga(k, :) = {baseFileName}; 
        %salvo dentro un vettore tutti i valori della distanza euclidea
        if strcmp(similarity,'D1')
            d(j,:) = abs((folderFeatures(j,:) - imageFeatures(1,:))/(1 + folderFeatures(j,:) + imageFeatures(1,:)));          
        else
            d(j,:) = pdist2(folderFeatures(j,:), imageFeatures(1,:), metric);
        end
    end
    [~, index]=sortrows(d);
    imageVector2 = imageVector(index);

    %calcolo rDBi in base alla classe dell'immagine sopra
    for k = 1 : eta
        classeRisultato = extractBetween(imageVector2{k}, "_", "."); 
        classeRisultato = str2num(classeRisultato{1:1});
        if(classeRisultato == classeImmagine)
            rDB = rDB + 1;
        end
    end                   
...
fprintf('\n\nIl valore di ARP è: %.2f %', ARP);
% ---------------------------------------------------------        


% ------------- Formula per il calcolo di ARR -------------        
for i = 1:length(immagini)
    rDB = 0;
    %recupero la classe dell'i-esima immagine del dataset
    classeImmagine = extractBetween(imageVector{i}, "_", "."); 
   ...
   % ---------------------------------------------------------
   % Formula per calcolare la distanza euclidea sulla base 
   % dell'iesima immagine dell'iterazione
   % ---------------------------------------------------------
    for j = 1:length(immagini)
        baseFileName = immagini(j).name;

        nomeRigaE(j, :) = {baseFileName}; %salvo dentro un vettore tutti i nomi delle immagini
        if strcmp(similarity,'D1')
            d(j, :) = abs((folderFeatures(j,:) - imageFeatures(1,:))/(1 + folderFeatures(j,:) + imageFeatures(1,:)));          
        else
            d(j, :) = pdist2(folderFeatures(j,:), imageFeatures(1,:), metric);
